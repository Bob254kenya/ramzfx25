import type { AuthInfo, DerivRestAccount, StoredCSRFToken, StoredCodeVerifier } from './types';

const CSRF_TOKEN_KEY    = 'oauth_csrf_token';
const CODE_VERIFIER_KEY = 'oauth_code_verifier';
const AUTH_INFO_KEY     = 'auth_info';
const REST_ACCOUNTS_KEY = 'deriv_rest_accounts';
const TOKEN_ACCOUNTS_KEY = 'deriv_accounts'; // legacy key — kept for backward compat
...
export function getCSRFToken(): string | null {
  ...
  if (Date.now() - stored.createdAt > TOKEN_MAX_AGE_MS) { clearCSRFToken(); return null; }
  return stored.value;
}
...
export function getCodeVerifier(): string | null {
  ...
  if (Date.now() - stored.createdAt > TOKEN_MAX_AGE_MS) { clearCodeVerifier(); return null; }
  return stored.value;
}
...
export function getAuthInfo(): AuthInfo | null {
  ...
  if (authInfo.expires_at && Date.now() > authInfo.expires_at * 1000) return null;
  return authInfo;
}
...
// --- REST Accounts (PKCE flow) ---

export function storeRestAccounts(accounts: DerivRestAccount[]): void {
  localStorage.setItem(REST_ACCOUNTS_KEY, JSON.stringify(accounts));
}

export function getRestAccounts(): DerivRestAccount[] | null {
  const raw = localStorage.getItem(REST_ACCOUNTS_KEY);
  if (!raw) return null;
  return JSON.parse(raw);
}

export function clearRestAccounts(): void {
  localStorage.removeItem(REST_ACCOUNTS_KEY);
}
...
export function clearAllAuthData(): void {
  ...
  // Also clear legacy token accounts
  localStorage.removeItem(TOKEN_ACCOUNTS_KEY);
